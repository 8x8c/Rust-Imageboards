...
/// Posts are loaded from the sled database (newest first) and displayed
...
async fn index(db: web::Data<Db>) -> impl Responder {
    ...
    // Build the full HTML document with responsive CSS.
    let mut html = String::new();
    html.push_str("<!DOCTYPE html><html><head><meta charset='utf-8'><title>Simple Imageboard</title>");
    html.push_str("<style>");
    html.push_str("body { font-family: Arial, sans-serif; background-color: #f8f8f8; margin: 0; padding: 20px; }");
    html.push_str(".container { width: 90%; max-width: 600px; margin: 0 auto; }");
    html.push_str("form { background: #fff; padding: 15px; border: 1px solid #ccc; border-radius: 4px; margin-bottom: 20px; }");
    html.push_str("form input[type='text'], form textarea { width: 100%; box-sizing: border-box; margin-bottom: 10px; padding: 8px; }");
    html.push_str("form input[type='submit'] { padding: 10px 20px; }");
    html.push_str(".post { background-color: #fff; border: 1px solid #ccc; border-radius: 4px; padding: 10px; margin-bottom: 20px; word-wrap: break-word; }");
    ...
    // Post form (subject and message are required)
    html.push_str(r#"
        <form action="/" method="post" enctype="multipart/form-data">
            <input type="text" name="subject" placeholder="Subject (max 30 chars)" required maxlength="30"><br>
            <textarea name="message" placeholder="Message (up to 20,000 characters)" rows="4" cols="50" required maxlength="20000"></textarea><br>
    ...
    html.push_str("</div>"); // Close container
...
/// Saves the file (if provided) to "./uploads" and stores the post in the sled database.
async fn post_handler(mut payload: Multipart, db: web::Data<Db>) -> actix_web::Result<HttpResponse> {
    ...
    // Validate message.
    let message = message.ok_or_else(|| actix_web::error::ErrorBadRequest("Message is required"))?;
...
/// On Unix, sets permissions to 0o755.
#[cfg(unix)]
